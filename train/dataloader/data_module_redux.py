...
class EditDataset(Dataset):
    def __init__(self, path, metadata_file, width_resize, height_resize, vae, tokenizer, tokenizer_2, text_encoder, text_encoder_2, device, preprocess=False):
        ...
        Args:
            ...
            noise_scheduler: Noise scheduler for generating noisy latents.
        ...
        self.image_encoder = SiglipVisionModel.from_pretrained("google/siglip-so400m-patch14-384", torch_dtype=torch.bfloat16).to(device)
    ...
    def _precompute_and_save(self):
        ...
        for item in tqdm(metadata_per_rank, desc=f"Rank {current_rank} Precomputing latents and embeddings", disable=not current_rank == 0):
            ...
            latent_data_path = input_image_path.with_suffix(f".redux384_latent_data_{self.width_resize}_{self.height_resize}.pt")
            ...
            cond_input = self.encode_image(input_image, self.device, 1)
    ...
    def __getitem__(self, idx):
        ...
        latent_data_path = (self.data_dir / item["input_image"]).with_suffix(f".redux384_latent_data_{self.width_resize}_{self.height_resize}.pt")
