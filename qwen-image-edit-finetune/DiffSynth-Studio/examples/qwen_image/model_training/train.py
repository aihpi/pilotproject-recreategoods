...
class QwenImageTrainingModule(DiffusionTrainingModule):
    ...
    def forward_preprocess(self, data):
        ...
        # Include segmentation mask if present (used only for loss computation)
        if "segmentation_mask" in data:
            inputs_shared["segmentation_mask"] = data["segmentation_mask"]
    ...
    def forward(self, data, inputs=None, return_inputs=False):
        ...
        if inputs is None:
            ...
            inputs = self.transfer_data_to_device(inputs, self.pipe.device)
        if return_inputs:
            return inputs
