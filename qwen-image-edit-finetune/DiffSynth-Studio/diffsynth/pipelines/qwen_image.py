...
class QwenImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.units = [
            ...
            QwenImageUnit_SegmentationMaskProcessor(),
    ...
    def training_loss(self, **inputs):
        ...
        # Handle both image noise prediction and segmentation mask prediction
        if isinstance(noise_pred, dict):
            # Dual output: image + segmentation
            image_noise_pred = noise_pred["image_noise"]
            seg_mask_pred = noise_pred.get("segmentation_mask", None)
            
            # Standard image loss
            image_loss = torch.nn.functional.mse_loss(image_noise_pred.float(), training_target.float())
            image_loss = image_loss * self.scheduler.training_weight(timestep)
            
            # Segmentation loss (if ground truth mask is available)
            if "segmentation_mask" in inputs and seg_mask_pred is not None:
                gt_seg_mask = inputs["segmentation_mask"]  # Ground truth mask
                # Ensure shapes match
                if gt_seg_mask.shape != seg_mask_pred.shape:
                    gt_seg_mask = torch.nn.functional.interpolate(
                        gt_seg_mask, size=seg_mask_pred.shape[-2:], mode='bilinear', align_corners=False
                    )
                
                # BCE loss for segmentation
                seg_loss = torch.nn.functional.binary_cross_entropy_with_logits(
                    seg_mask_pred.float(), gt_seg_mask.float()
                )
                
                # Combined loss (image loss + weighted segmentation loss)
                mask_loss_weight = getattr(self, 'mask_loss_weight', 1.0)
                total_loss = image_loss + mask_loss_weight * seg_loss
                return total_loss
            else:
                # No segmentation mask available, return just image loss
                return image_loss
        else:
            # Single output: only image noise prediction
...
class QwenImageUnit_SegmentationMaskProcessor(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("segmentation_mask", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: QwenImagePipeline, segmentation_mask, tiled, tile_size, tile_stride):
        """
        Process segmentation mask through VAE if available.
        
        Args:
            pipe: The QwenImagePipeline instance
            segmentation_mask: Binary segmentation mask tensor (B, 1, H, W) or None
            tiled: Whether to use tiling
            tile_size: Size of tiles
            tile_stride: Stride between tiles
            
        Returns:
            dict: Contains segmentation_latents if mask was processed
        """
        ...
        # Ensure segmentation mask has correct format
        if segmentation_mask.dim() == 3:
            segmentation_mask = segmentation_mask.unsqueeze(1)  # Add channel dimension
        ...
        if hasattr(pipe.vae, 'encode_segmentation_mask'):
            ...
            # Fallback: resize mask to latent space dimensions
...
class QwenImageUnit_EditImageEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("edit_image", "height", "width", "tiled", "tile_size", "tile_stride", "edit_image_auto_resize"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: QwenImagePipeline, edit_image, height, width, tiled, tile_size, tile_stride, edit_image_auto_resize):
        if edit_image is None:
            return {}
        pipe.load_models_to_device(['vae'])
        ...
        if edit_image_auto_resize:
            ...
            if original_width * original_height > 0:
                scale_factor = (target_area / (original_width * original_height)) ** 0.5
                new_width = int(original_width * scale_factor)
                new_height = int(original_height * scale_factor)
                
                # Resize to target dimensions
                edit_image = edit_image.resize((new_width, new_height), Image.LANCZOS)
        
        # Process the edit image
        edit_image = pipe.preprocess_image(edit_image.resize((width, height))).to(device=pipe.device, dtype=pipe.torch_dtype)
...
def model_fn_qwen_image(
    ...
    **kwargs
    ...
    # Ensure timestep is properly formatted before processing
    if timestep.dim() > 1:
        timestep = timestep.squeeze(0)  # Remove batch dimension if present
    elif timestep.dim() == 0:
        timestep = timestep.unsqueeze(0)  # Add batch dimension for consistency
    ...
    # Handle segmentation output from DiT
    if hasattr(dit, 'enable_segmentation') and dit.enable_segmentation:
        # If DiT returns dict with both image and segmentation
