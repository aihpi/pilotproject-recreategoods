...
class QwenImageDiT(torch.nn.Module):
    def __init__(
        ...
        if self.enable_segmentation:
            self.seg_norm_out = AdaLayerNorm(3072, single=True)
            self.seg_proj_out = nn.Linear(3072, 1)  # Single channel for binary segmentation
    ...
    def forward(
        ...
        # Return both image and segmentation if enabled
        if self.enable_segmentation:
            # Predict segmentation mask from the shared representation
            seg_features = self.seg_norm_out(image, conditioning)
            seg_logits = self.seg_proj_out(seg_features)
            seg_latents = rearrange(seg_logits, "B (H W) (C P Q) -> B C (H P) (W Q)", H=height//16, W=width//16, P=2, Q=2)
            return {"image_noise": latents, "segmentation_mask": seg_latents}
        else:
