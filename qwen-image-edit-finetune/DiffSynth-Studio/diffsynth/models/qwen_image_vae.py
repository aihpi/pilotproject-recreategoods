...
class QwenImageVAE(torch.nn.Module):
    ...
    def encode_segmentation_mask(self, mask, **kwargs):
        """
        Encode a binary segmentation mask into the same latent space as images.
        
        Args:
            mask (torch.Tensor): Binary segmentation mask of shape (B, 1, H, W) or (B, H, W)
            
        Returns:
            torch.Tensor: Encoded mask in latent space, shape (B, 16, H//8, W//8)
        """
        if mask.dim() == 3:
            # Add channel dimension if missing
            mask = mask.unsqueeze(1)
        
        # Normalize mask to [0, 1] and convert to RGB-like format for VAE
        # Convert binary mask (0/1) to RGB format that VAE expects (3 channels)
        mask_3ch = mask.repeat(1, 3, 1, 1)  # Repeat single channel to 3 channels
        
        # Encode through VAE encoder to get latent representation
        return self.encode(mask_3ch, **kwargs)
    
    def decode_segmentation_latent(self, latent, **kwargs):
        """
        Decode segmentation mask from latent representation back to binary mask.
        
        Args:
            latent (torch.Tensor): Latent representation of shape (B, 16, H//8, W//8)
            
        Returns:
            torch.Tensor: Binary segmentation mask of shape (B, 1, H, W) with values in [0, 1]
        """
        # Decode through VAE decoder
        decoded = self.decode(latent, **kwargs)  # Shape: (B, 3, H, W)
        
        # Convert back to single channel binary mask
        mask = decoded.mean(dim=1, keepdim=True)  # Average across RGB channels
    ...
    def encode_joint(self, image, mask):
        """
        Encode both image and segmentation mask jointly through the VAE.
        
        Args:
            image (torch.Tensor): RGB image of shape (B, 3, H, W)
            mask (torch.Tensor): Binary mask of shape (B, 1, H, W) or (B, H, W)
            
        Returns:
            tuple: (image_latent, mask_latent) both in shape (B, 16, H//8, W//8)
        """
        image_latent = self.encode(image)
        mask_latent = self.encode_segmentation_mask(mask)
