...
from utils.flux_processor import CustomFluxAttnProcessor2_0, create_controller
...
class Prompt2PromptPipeline(FluxPipeline):
    ...
    r"""
    Args:
    Prompt-to-Prompt-Pipeline for text-to-image generation using FLUX. This model inherits from the Flux pipeline for text-to-image generation.
    ...
    def __call__(
        ...
        if joint_attention_kwargs is None:
            joint_attention_kwargs = {}  # Default to empty dictionary
        # Check if `prompt` is valid
        if not isinstance(prompt, list) or not prompt:
            raise ValueError("`prompt` must be a non-empty list with at least one string element.")

        # self.controller = create_controller(joint_attention_kwargs, self.tokenizer)
        from utils.freeprompt import SelfAttentionControlEdit
        self.controller = create_controller(prompts=prompt, num_inference_steps=num_inference_steps,joint_attention_kwargs=joint_attention_kwargs, tokenizer=self.tokenizer)
        # self.controller = SelfAttentionControlEdit(prompts=prompt, num_steps=num_inference_steps, self_replace_steps=.8)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                # apply_p2p = timesteps is not None and p2p_threshold > (i / (len(timesteps) - 1))
                # if apply_p2p:
                #     for module in self.transformer.modules():
                #         if isinstance(module, CustomFluxTransformerBlock):
                #             module.prompt_to_prompt = True
                #             module.p2p_strength = 0.4
    ...
    def register_attention_control(self, controller):
        ...
        for name in self.transformer.attn_processors.keys():
            att_count += 1
            attn_procs[name] = CustomFluxAttnProcessor2_0(controller=controller)
